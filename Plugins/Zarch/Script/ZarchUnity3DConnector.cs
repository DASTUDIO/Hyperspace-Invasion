...
namespace Z
{
    public class ZarchUnity3DConnector : MonoBehaviour
    {
        ...
        public class GoHelper
        {
            ...
            // ui
            public void image(Sprite s) { var r = go.GetComponent<Image>(); if (r) { r.sprite = s; } else { go.GetComponentInChildren<Image>().sprite = s; } }

            public Sprite image() { var r = go.GetComponent<Image>(); if (r) { return r.sprite; } return null; }

            public void text(string txt) { var r = go.GetComponent<Text>(); if (r) { r.text = txt; } else { go.GetComponentInChildren<Text>().text = txt; } }

            public string text() { var r = go.GetComponent<Text>(); if (r) { return r.text; }  return ""; }

            public void slider(float progress) { var r = go.GetComponent<Slider>(); if (r) { r.value = progress; } else { go.GetComponentInChildren<Slider>().value = progress; } }

            public float slider() { var r = go.GetComponent<Slider>(); if (r) { return r.value; }  return 0f; }
        ...
        public class TaskHelper
        {
            ...
            {
                instance.callStop = true;
                float _delay = (float)System.Convert.ToDouble(delay);
                int _loop = System.Convert.ToInt32(isLoop);
                instance.tmp_coroutines.Add(func);
                instance.tmp_params.Add(new KeyValuePair<float, int>(_delay, _loop));
            }

            #region something i have to repeat
            ...
            public void coroutine2(System.Func<object[], object> func, object delay, object isLoop)
            {
                instance.callStop2 = true;
                float _delay = (float)System.Convert.ToDouble(delay);
                int _loop = System.Convert.ToInt32(isLoop);
                instance.tmp_coroutines2.Add(func);
                instance.tmp_params2.Add(new KeyValuePair<float, int>(_delay, _loop));
            }

            public void coroutine3(System.Func<object[], object> func, object delay, object isLoop)
            {
                instance.callStop3 = true;
                float _delay = (float)System.Convert.ToDouble(delay);
                int _loop = System.Convert.ToInt32(isLoop);
                instance.tmp_coroutines3.Add(func);
                instance.tmp_params3.Add(new KeyValuePair<float, int>(_delay, _loop));
            }
        ...
        #region somethine i have to repeat
        ...
        List<System.Func<object[], object>> tmp_coroutines2 = new List<System.Func<object[], object>>();
        List<KeyValuePair<float, int>> tmp_params2 = new List<KeyValuePair<float, int>>();

        List<System.Func<object[], object>> tmp_coroutines3 = new List<System.Func<object[], object>>();
        List<KeyValuePair<float, int>> tmp_params3 = new List<KeyValuePair<float, int>>();
        ...
        void Update()
        {
            if (callStop) { StopCoroutine("CoroutineTask"); callStop = false; }
            if (callStop1) { StopCoroutine("CoroutineTask1"); callStop1 = false; }
            if (callStop2) { StopCoroutine("CoroutineTask2"); callStop2 = false; }
            if (callStop3) { StopCoroutine("CoroutineTask3"); callStop3 = false; }

            if (callStopAll) { StopAllCoroutines(); callStopAll = false; }

            if (tmp_coroutines.Count != 0)
            {
            ...
            if (tmp_coroutines2.Count != 0)
            {
                for (int i = 0; i < tmp_coroutines2.Count; i++)
                {
                    var pms = tmp_params2[i];
                    var func = tmp_coroutines2[i];

                    StartCoroutine(CoroutineTask2(func, pms.Key, pms.Value));

                    tmp_coroutines2.Remove(func);
                    tmp_params2.Remove(pms);
                }
            }

            if (tmp_coroutines3.Count != 0)
            {
                for (int i = 0; i < tmp_coroutines3.Count; i++)
                {
                    var pms = tmp_params3[i];
                    var func = tmp_coroutines3[i];

                    StartCoroutine(CoroutineTask3(func, pms.Key, pms.Value));

                    tmp_coroutines3.Remove(func);
                    tmp_params3.Remove(pms);
                }
            }
        ...
        IEnumerator CoroutineTask2(System.Func<object[], object> action, float delay, int loopTimes)
        {
            if (loopTimes < 0) // looper
            {
                while (true)
        ...
        IEnumerator CoroutineTask3(System.Func<object[], object> action, float delay, int loopTimes)
        {
            if (loopTimes < 0) // looper
            {
                while (true)
    ...
    public static class ZarchUnityExtension
    {
        ...
        public static void S_coroutine2(this Zarch.Extension ze, System.Action action, float delay = 0, int loopTimes = 0)
        {
            ZarchUnity3DConnector.instance.task.coroutine2((pms) => { action(); return null; }, delay, loopTimes);
        }

        public static void S_coroutine3(this Zarch.Extension ze, System.Action action, float delay = 0, int loopTimes = 0)
        {
            ZarchUnity3DConnector.instance.task.coroutine3((pms) => { action(); return null; }, delay, loopTimes);
        }
